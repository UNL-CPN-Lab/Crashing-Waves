...
e = zeros(size(theta));
for i = 1:length(theta)-4
    n_i = pathloss_exp(pl_up_car_updated(i,:),d_range);
    PL = pl_up_car_updated(i,1) + 10.*n_i.*log10(d_range./d_ref);
    e(i) =  std(pl_up_car_updated(i,:) -PL);
end

sdev = mean(e);

save pathloss_data_high_ref.mat V_high n_high C_high sdev
