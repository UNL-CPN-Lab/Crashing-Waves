...
% 
% d_NJPCB7_180 = ddd;
% PL_NJPCB7_180 = Gain - RSS(start:end_)';
% d_PL_NJPCB7_180 = d_PL;
% PL_corrected_NJPCB7_180  = PL_corrected;
% save PL_d_NJPCB7_180.mat d_NJPCB7_180 PL_NJPCB7_180 d_PL_NJPCB7_180 PL_corrected_NJPCB7_180 start end_

% error(isnan(error)) = [];
% bins = min(error):5:max(error);  % define some bins that cover the range of interest
% assignments = discretize(error,bins);  % get a vector telling you which bin each price is in
% bar(assignments)
...
% print('Results/NJPCB7_180_Normal_Hist','-dpng');
% 
% figure;
% h2 = histfit(error.^2,50,'rician');
% print('Results/NJPCB7_180_Rician_Hist','-dpng');
% 
% figure;
% h3 = histfit(error.^2,50,'rayleigh');
% print('Results/NJPCB7_180_Rayleigh_Hist','-dpng');
